...
import {
  Sun, Moon, Wifi, Volume2, VolumeX, Terminal, FolderOpen,
  Globe, ChevronLeft, ChevronRight, MoreHorizontal, Mail, FileText,
  type LucideIcon,
} from "lucide-react";
...
export default function Taskbar({
  ...
  const toggleCalendar = () => { const next = !showCalendar; closeAll(); if (next) setShowCalendar(true); };
  const toggleWifi     = () => { const next = !showWifi;     closeAll(); if (next) setShowWifi(true); };
  const toggleVolume   = () => { const next = !showVolume;   closeAll(); if (next) setShowVolume(true); };
  const toggleLang     = () => { const next = !showLang;     closeAll(); if (next) setShowLang(true); };

  useEffect(() => {
    if (!isMobile || !showVolume) return;
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     MOBILE / TABLET — iOS-style dock + slide-up control center
     ═══════════════════════════════════════════════════════════════════════════ */
  if (isMobile) {
    const dockApps = [
    ...
    return (
      <div ref={taskbarRef} style={{ position: "absolute", left: 0, right: 0, bottom: 0, zIndex: 40, pointerEvents: "none" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {dockApps.map((app) => {
              const Icon = app.icon;
              const isWork = app.id === "work";
              return (
                <button
                  key={app.id}
                  onClick={() => {
                    if (isWork) onOpenWork();
                    else if (app.id === "terminal") onOpenTerminal();
                    else onOpenApp?.(app.id);
                  }}
                  className="flex flex-col items-center gap-1"
                  style={{ background: "none", border: "none", cursor: "pointer", width: 50, position: "relative" }}
                >
                  {/* Pulse ring on Work icon when onboard is active */}
                  {isWork && highlightWork && (
                    <div style={{
                      position: "absolute",
                      top: -3, left: "50%",
                  ...
                  <div
                    ...
                    style={{ width: 46, height: 46, display: "flex", alignItems: "center", justifyContent: "center" }}
                  >
                    <div className="icon-frame" style={{
                      width: 46, height: 46, display: "flex", alignItems: "center", justifyContent: "center",
                      borderRadius: 14, overflow: "hidden",
                    }}>
                      {Icon && <Icon size={22} strokeWidth={1.3} style={{ color: "var(--desktop-icon-text)" }} />}
                    </div>
                  </div>
                  <span className="icon-label" style={{
                    fontFamily: "sans-serif", fontSize: 9, fontWeight: 500,
                    color: "var(--desktop-icon-text)", textAlign: "center",
            ...
            {/* Separator */}
            <div style={{ width: 1, height: 32, background: "var(--separator)", flexShrink: 0 }} />

            {/* More button */}
            <button
              onClick={() => setShowDrawer(d => !d)}
              className="flex flex-col items-center gap-1"
              style={{ background: "none", border: "none", cursor: "pointer", width: 50 }}
            >
              <div className="desktop-icon" style={{ width: 46, height: 46, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div className="icon-frame" style={{ width: 46, height: 46, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 14 }}>
                  <MoreHorizontal size={22} strokeWidth={1.3} style={{ color: "var(--desktop-icon-text)" }} />
                </div>
              </div>
              <span className="icon-label" style={{
                fontFamily: "sans-serif", fontSize: 9, fontWeight: 500,
                color: "var(--desktop-icon-text)", textAlign: "center", lineHeight: 1,
              }}>
                More
              </span>
            </button>
          ...
          {showDrawer && (
            <>
              <motion.div
                ...
                style={{ zIndex: -1, background: "rgba(0,0,0,0.3)", pointerEvents: "auto" }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: isDark
                    ...
                    : "0 8px 32px rgba(0,0,0,0.10), inset 0 1px 0 rgba(255,255,255,0.7)",
                }}
              ...
              >
                <div style={{ padding: "14px 16px 16px" }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                    {[
                      { id: "gimp", label: "GIMP", img: "/gimp-logo.webp" },
                      { id: "resume", label: "Resume", icon: FileText },
                    ].map((app) => {
                      const Icon = app.icon;
                      return (
                        <button key={app.id} onClick={() => { onOpenApp?.(app.id); setShowDrawer(false); }}
                          className="flex flex-col items-center gap-1.5"
                          style={{ background: "none", border: "none", cursor: "pointer", width: 52 }}>
                          <div className="desktop-icon" style={{ width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <div className="icon-frame" style={{ width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 12, overflow: "hidden" }}>
                              {"img" in app && app.img ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={app.img} alt={app.label} width={24} height={24} style={{ objectFit: "contain" }} />
                              ) : Icon ? (
                                <Icon size={18} strokeWidth={1.3} style={{ color: "var(--desktop-icon-text)" }} />
                              ) : null}
                            </div>
                          </div>
                          <span className="font-mono text-[9px]" style={{ color: "var(--text-secondary)" }}>{app.label}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div className="font-mono text-[9px] uppercase tracking-widest mb-2" style={{ color: "var(--text-tertiary)", letterSpacing: "0.14em" }}>
                      Find me online
                    </div>
                    <div style={{ borderRadius: 12, overflow: "hidden", background: "var(--taskbar-icon-active-bg)", border: "1px solid var(--taskbar-border)" }}>
                      {SOCIAL_LINKS.map((s, i) => (
                        <a key={s.id} href={s.url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 px-3 py-2.5"
                          style={{ textDecoration: "none", borderBottom: i < SOCIAL_LINKS.length - 1 ? "1px solid var(--separator)" : "none" }}>
                          <div className="flex items-center justify-center rounded-lg flex-shrink-0"
                            style={{ width: 28, height: 28, background: s.id === "github" ? "rgba(255,255,255,0.08)" : `${s.color}22`, color: s.id === "github" ? "var(--text-primary)" : s.color, border: `1px solid ${s.id === "github" ? "var(--window-border)" : `${s.color}44`}` }}>
                            {s.icon}
                          </div>
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     DESKTOP — vertical left sidebar (unchanged)
     ═══════════════════════════════════════════════════════════════════════════ */
  return (
    <div ref={taskbarRef} className="taskbar absolute left-0 top-0 bottom-0 z-40 w-14 flex flex-col items-center justify-between py-3">
      <div className="flex flex-col items-center gap-2">
        <button onClick={onOpenAbout} className="taskbar-icon flex items-center justify-center rounded-full overflow-hidden"
          style={{ width: 36, height: 36, flexShrink: 0, backgroundImage: "url('/profile.png')", backgroundSize: "cover", backgroundPosition: "center 20%" }} title="About Me" />
        ...
        <div style={{ position: "relative" }}>
          ...
          <button onClick={onOpenWork} className="taskbar-icon rounded-xl flex items-center justify-center" style={{ width: 38, height: 38 }} title="Work">
            <FolderOpen size={16} strokeWidth={1.5} />
          </button>
        </div>
        <button onClick={onOpenTerminal} className="taskbar-icon rounded-xl flex items-center justify-center" style={{ width: 38, height: 38 }} title="Terminal">
      ...
      <div className="flex flex-col items-center gap-1.5 flex-1 justify-center py-3">
        {minimizedWindows.map((win) => {
          const Icon = win.icon;
          return (
            <button key={win.id} onClick={() => onRestoreWindow(win.id)}
              className="taskbar-icon rounded-lg flex flex-col items-center justify-center relative"
              style={{ width: 38, height: 38 }} title={`Restore — ${win.title}`}>
      ...
      <div className="flex flex-col items-center gap-1.5">
        <button onClick={onToggleTheme} className="taskbar-icon rounded-xl flex items-center justify-center" style={{ width: 38, height: 38 }} title={isDark ? "Switch to light" : "Switch to dark"}>
        ...
        <div className="relative">
          <button onClick={() => { onToggleMute(); toggleVolume(); }} className="taskbar-icon rounded-lg flex items-center justify-center" style={{ width: 34, height: 34 }} title={isMuted ? "Unmute" : "Mute"}>
            {isMuted ? <VolumeX size={14} strokeWidth={1.5} /> : <Volume2 size={14} strokeWidth={1.5} />}
          </button>
          {showVolume && (
            <div className="absolute z-50 rounded-xl font-mono text-[10px] leading-snug whitespace-nowrap"
              style={{ left: 42, bottom: 0, padding: "8px 12px", background: "var(--window-bg)", border: "1px solid var(--window-border)", color: "var(--text-secondary)", backdropFilter: "blur(32px)", boxShadow: "0 8px 24px rgba(0,0,0,0.14)" }}>
              <span style={{ color: "var(--text-tertiary)" }}>Now playing —</span> {isMuted ? "muted" : "Techno Jazz Ambient Mix"}
            </div>
          )}
        </div>
          ...
          {showLang && (
            <div className="absolute z-50 rounded-xl overflow-hidden"
              ...
              {(["en", "es"] as const).map((lang) => (
                <button key={lang} onClick={() => { onSetLocale(lang); setShowLang(false); }}
